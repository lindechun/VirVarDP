...
def gatherSubstitution(Ipath,ama,orderGene):
    if not len(glob(Ipath+"/*/*.nonsynonymous.AmAcid.substitution.txt")):
        sys.exit('There isn\'t exist anyone syonymous substitution results of every gene')

    for i in orderGene:
        Gene_syno_file=Ipath+'/'+i+'/'+i+'.nonsynonymous.AmAcid.substitution.txt'

        if os.path.exists(Gene_syno_file):
            table=pd.read_table(Gene_syno_file,sep="\t",index_col=0,header=0)
            table.columns=map(lambda x: int(x)+ama[i][0]-1,table.columns)
            table.columns = pd.MultiIndex.from_tuples([tuple([i,x]) for x in table.columns])
